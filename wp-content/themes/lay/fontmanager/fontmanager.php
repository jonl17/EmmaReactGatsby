...
class FontManager{
	...
	public function __construct(){
		...
		add_filter( 'upload_mimes', array( $this, 'allow_woff'), 1, 1);
			...
			if (is_array(FontManager::$customFonts)) {
				for($i=0; $i<count(FontManager::$customFonts); $i++){
					if(!array_key_exists('type', FontManager::$customFonts[$i]) || FontManager::$customFonts[$i]['type'] == "attachment" ){
						...
						$newfontsCSS .= '@font-face{ font-family: "'.FontManager::$customFonts[$i]['fontname'].'"; src: url("'.$url.'") format("woff"); } ';
					}
				}
			}
	...
	public function allow_woff($mime_types){
	...
	public function fontmanager_scripts($hook){
		if ( $hook == 'lay-options_page_manage-webfonts' ) {
			...
			wp_enqueue_script( 'fontmanager-app', Setup::$uri."/fontmanager/assets/js/fontmanager.app.min.js", array( 'marionettev2', 'jquery', 'underscore', 'backbone', 'json2' ), Setup::$ver );
