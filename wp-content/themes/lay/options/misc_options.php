...
class MiscOptions{
	...
	public function misc_options_settings_api_init(){
		...
		register_setting( 'manage-miscoptions', 'lay_tablet_breakpoint' );
 	 	...
 	 	// other section
 	 	add_settings_field(
 			'misc_options_force_custom_links_target_blank',
 			'Menu links of type "Custom Links" always open in new tab',
 			array($this, 'misc_setting_force_custom_links_target_blank'),
 			'manage-miscoptions',
 			'other_section'
 		);
 	 	register_setting( 'manage-miscoptions', 'misc_options_force_custom_links_target_blank' );
	...
	public function misc_setting_image_loading(){
		...
		$checked1 = $val == 'lazy_load' ? 'checked' : '';
		$checked2 = $val == 'instant_load' ? 'checked' : '';

		echo '<input type="radio" name="misc_options_image_loading" id="lazy_load" value="lazy_load" '.$checked1.'><label for="lazy_load">Lazy load an image when you scroll to it</label><br>';
		echo '<input type="radio" name="misc_options_image_loading" id="instant_load" value="instant_load" '.$checked2.'><label for="instant_load">Load all images of a page instantly</label>';
	}
	...
	public function misc_setting_lay_tablet_breakpoint(){
		...
		Must be bigger than phone breakpoint. Important for "Thumbnailgrid" and "Elementgrid".</label>';		
	}
	...
	public function misc_setting_navigation_transition_duration(){
		$value = get_option( 'misc_options_navigation_transition_duration', '0.6' );
	...
	public function misc_setting_force_custom_links_target_blank(){
		$val = get_option('misc_options_force_custom_links_target_blank', 'on');
		$checked = "";
		if( $val == "on" ){
			$checked = "checked";
		}
		echo '<input type="checkbox" name="misc_options_force_custom_links_target_blank" id="misc_options_force_custom_links_target_blank" '.$checked.'>';
	}
